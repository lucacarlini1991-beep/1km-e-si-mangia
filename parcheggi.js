...
(function () {
  ...
  async function searchParking(exit){
    ...
    try{
      ...
      const seen=new Set();
      state.parking=(data.elements||[]).map(e=>normalize(e,exit)).filter(Boolean).filter(x=>x.distance<=RADIUS&&!seen.has(x.id)&&seen.add(x.id)).sort((a,b)=>(a.distance-b.distance)).slice(0,100);
      renderParking(exit);renderList(exit);
      status(state.parking.length+' parcheggi trovati · '+(exit.nome||'uscita'));
    }catch(e){
      console.error(e);state.parking=[];renderParking(exit);renderList(exit);status('Servizio parcheggi non disponibile · riprova',true);
    }finally{state.loading=false;busy($('mpNearestExit'),false,'','🛣️ CERCA VICINO ALL\'USCITA');}
  }
  ...
  function locate(){
    const b=$('mpLocate');
    if(!navigator.geolocation){status('La geolocalizzazione non è disponibile su questo dispositivo',true);return;}
    busy(b,true,'📍 CERCO LA POSIZIONE…','📍 USA LA MIA POSIZIONE');
    navigator.geolocation.getCurrentPosition(pos=>{
      const here={lat:pos.coords.latitude,lon:pos.coords.longitude};let best=null,bestD=Infinity;
      state.exits.forEach(e=>{const d=distance(here,{lat:Number(e.lat),lon:Number(e.lon)});if(d<bestD){bestD=d;best=e;}});
      if(best){state.map.flyTo([Number(best.lat),Number(best.lon)],13,{duration:.8});searchParking(best);status('Posizione trovata · '+best.nome);}
      else status('Nessuna uscita trovata',true);
      busy(b,false,'','📍 USA LA MIA POSIZIONE');
    },err=>{console.warn(err);status(err.code===1?'Posizione negata dal browser':'Posizione non disponibile',true);busy(b,false,'','📍 USA LA MIA POSIZIONE');},{enableHighAccuracy:true,timeout:20000,maximumAge:0});
  }

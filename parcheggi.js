...
(function () {
  ...
  function renderList(exit){
    ...
    el.innerHTML=state.parking.map((x,i)=>`<article class="mp-card"><h3>🚛 ${esc(nomeParcheggio(x))}</h3><div class="mp-meta"><span class="mp-chip">📍 ${fmt(x.distance)} dall’uscita</span><span class="mp-chip ${x.compat===true?'good':x.compat===false?'bad':'warn'}">${compatText(x)}</span></div><div class="mp-services">${services(x.tags).length?esc(services(x.tags).join(' · ')):'Servizi non indicati'}${Object.values(x.limits).some(Boolean)?'<br>'+Object.entries({'H':x.limits.height,'Larg.':x.limits.width,'Lung.':x.limits.length,'Peso':x.limits.weight}).filter(([,v])=>v).map(([k,v])=>k+' '+esc(v)).join(' · '):''}</div><div class="mp-review" data-review="${i}">⭐ Caricamento recensioni…</div><div class="mp-card-actions"><button class="mp-btn dark" type="button" data-nav-index="${i}">🧭 NAVIGA</button><button class="mp-btn" type="button" data-map-index="${i}">📍 MAPPA</button></div></article>`).join('');document.querySelectorAll('[data-review]').forEach(async e=>{const i=+e.dataset.review,x=state.parking[i];if(!window.ParkingReviews){e.textContent='⭐ Recensioni non disponibili';return}const a=await window.ParkingReviews.summary(x);e.innerHTML=`<b>⭐ ${a.avg||'—'}</b> ${a.count?'· '+a.count+' recensioni':'· Nessuna recensione'}<br><button type="button" data-write-review="${i}">⭐ SCRIVI UNA RECENSIONE</button>`;e.querySelector('button').onclick=()=>window.ParkingReviews.open(x)});
  }

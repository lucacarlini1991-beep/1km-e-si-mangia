...
(function(){
  ...
  function show(exit,items,googleSearched=false,radius=1000){
    ...
    const filterBar='<div style="position:relative;z-index:2;display:flex;gap:7px;overflow-x:auto;padding:10px 12px 12px;margin:0;background:#fff;border-bottom:1px solid #edf1ef;-webkit-overflow-scrolling:touch;scroll-snap-type:x proximity">'+filters.map(([label,value])=>'<button type="button" data-distance-filter="'+value+'" style="box-sizing:border-box;flex:0 0 auto;min-height:38px;border:1px solid '+(String(radius)===String(value)?'#075c3b':'#cbd8d2')+';border-radius:20px;background:'+(String(radius)===String(value)?'#075c3b':'#fff')+';color:'+(String(radius)===String(value)?'#fff':'#075c3b')+';padding:8px 13px;font-weight:800;font-size:12px;line-height:1;cursor:pointer;white-space:nowrap;scroll-snap-align:start">'+label+'</button>').join('')+'</div>';
    close();
    ...
    const googleButton=googleSearched
      ? `<div style="position:relative;z-index:1;margin:10px 12px 0;padding:9px 12px;border-radius:11px;background:#eef6f1;color:#075c3b;text-align:center;font-size:12px;font-weight:800">✓ Risultati Google Places aggiunti</div>`
      : `<button id="cercaAltriGoogle" type="button" style="position:relative;z-index:1;margin:10px 12px 0;border:1px solid #075c3b;border-radius:11px;background:#fff;color:#075c3b;padding:10px 12px;font-weight:800;font-size:12px;letter-spacing:.2px;cursor:pointer">🔎 CERCA ALTRI RISTORANTI CON GOOGLE</button>`;

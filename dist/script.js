...
function bloccoRistoranteExtra(ristorante) {
  ...
  return `
    <div style="margin-top:9px;padding-top:9px;border-top:1px solid #e5e5e5;display:grid;gap:5px">
      ...
      <div style="font-size:13px"><strong>${stelleHtml(valutazione.media)}</strong> <span style="color:#66736f">${valutazione.media != null ? valutazione.media + " / 5 · " + valutazione.totale + " " + (window.I18N?.tr("ratings","valutazioni")||"valutazioni") : (window.I18N?.tr("ratingNone","Nessuna valutazione")||"Nessuna valutazione")}</span></div>
      <button type="button" data-recensione-id="${escapeHtml(ristorante.id || ristorante.osm_id || ristorante.nome)}" style="margin-top:3px;border:0;border-radius:9px;background:#f5a719;color:#073f2e;padding:8px 10px;font-weight:800;cursor:pointer">${window.I18N?.tr("rateInfo","⭐ VALUTA / LASCIA UN’INFORMAZIONE")||"⭐ VALUTA / LASCIA UN’INFORMAZIONE"}</button>
    </div>`;
}
...
function apriRecensione(ristorante) {
  ...
  modal.innerHTML = `<div style="width:min(94vw,520px);max-height:88vh;overflow:auto;background:#fff;border-radius:18px;padding:20px;box-shadow:0 20px 60px rgba(0,0,0,.35)"><div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><div><div style="font-size:12px;color:#f5a719;font-weight:800;letter-spacing:2px">1 KM E SI MANGIA</div><h2 style="margin:4px 0">${escapeHtml(ristorante.nome || "Ristorante")}</h2></div><button id="chiudiRecensione1km" type="button" style="border:0;background:#eee;border-radius:50%;width:36px;height:36px;font-size:20px">×</button></div><div data-online-summary style="margin:12px 0 18px;font-size:14px">${valutazione.media != null ? `${stelleHtml(valutazione.media)} <strong>${valutazione.media}/5</strong> · ${valutazione.totale} valutazioni` : (window.I18N?.tr("noRatingYet","Ancora nessuna valutazione")||"Ancora nessuna valutazione")}</div><div style="padding:14px;background:#f4f7f5;border-radius:12px"><strong>${window.I18N?.tr("yourExperience","La tua esperienza")||"La tua esperienza"}</strong><div id="stelleInput1km" style="font-size:30px;letter-spacing:3px;margin:7px 0;cursor:pointer" aria-label="${window.I18N?.tr("chooseStars","Scegli da 1 a 5 stelle")||"Scegli da 1 a 5 stelle"}">☆☆☆☆☆</div><textarea id="testoRecensione1km" maxlength="500" placeholder="${window.I18N?.tr("reviewPlaceholder","Es. parcheggio comodo, servizio veloce, adatto a camion...")||"Es. parcheggio comodo, servizio veloce, adatto a camion..."}" style="width:100%;min-height:90px;border:1px solid #ccd8d3;border-radius:10px;padding:10px;font:inherit;resize:vertical"></textarea><button id="salvaRecensione1km" type="button" style="width:100%;margin-top:10px;border:0;border-radius:10px;background:#075c3b;color:#fff;padding:12px;font-weight:800">${window.I18N?.tr("publishAnon","PUBBLICA ANONIMAMENTE")||"PUBBLICA ANONIMAMENTE"}</button></div><div data-online-list style="margin-top:18px"><strong>${window.I18N?.tr("userExperiences","Esperienze degli utenti")||"Esperienze degli utenti"}</strong>${recensioni.length ? recensioni.slice(-8).reverse().map(r => `<div style="border-bottom:1px solid #e7ece9;padding:12px 0"><div>${stelleHtml(Number(r.stelle))}</div><div style="margin-top:4px;color:#53635e">${escapeHtml(r.testo || "")}</div><small style="color:#8a9692">${window.I18N?.tr("anonymousUser","Utente anonimo")||"Utente anonimo"} · ${escapeHtml(r.data || "")}</small></div>`).join("") : `<p style="color:#687772">${window.I18N?.tr("noExperience","Nessuna esperienza ancora. Sii il primo.")||"Nessuna esperienza ancora. Sii il primo."}</p>`}</div><small style="display:block;margin-top:15px;color:#8a9692">${window.I18N?.tr("reviewPrivacy","Le informazioni vengono pubblicate in forma anonima e sono condivise con gli altri utenti.")||"Le informazioni vengono pubblicate in forma anonima e sono condivise con gli altri utenti."}</small></div>`;
  ...
  caricaRecensioniOnline(ristorante).then(() => {
    ...
    if (summary) summary.innerHTML = latestVal.media != null ? `${stelleHtml(latestVal.media)} <strong>${latestVal.media}/5</strong> · ${latestVal.totale} valutazioni` : (window.I18N?.tr("noRatingYet","Ancora nessuna valutazione")||"Ancora nessuna valutazione");
    const list = modal.querySelector("[data-online-list]");
    if (list) list.innerHTML = latest.length ? latest.slice(0,8).map(r => `<div style="border-bottom:1px solid #e7ece9;padding:12px 0"><div>${stelleHtml(Number(r.stelle))}</div><div style="margin-top:4px;color:#53635e">${escapeHtml(r.testo || "")}</div><small style="color:#8a9692">${window.I18N?.tr("anonymousUser","Utente anonimo")||"Utente anonimo"} · ${escapeHtml(r.data || "")}</small></div>`).join("") : `<p style="color:#687772">${window.I18N?.tr("noExperience","Nessuna esperienza ancora. Sii il primo.")||"Nessuna esperienza ancora. Sii il primo."}</p>`;
  });
  ...
  modal.querySelector("#salvaRecensione1km").onclick = async () => {
    ...
    try {
      ...
      console.error(window.I18N?.tr("reviewError","Errore nella pubblicazione della recensione.")||"Errore nella pubblicazione della recensione.", error);

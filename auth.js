(function(){
  ...
  .auth-box button{width:100%;padding:13px;border:0;border-radius:10px;background:#075c3b;color:#fff;font:800 16px inherit;cursor:pointer;margin-top:8px}.auth-box .alt{background:#edf3ef;color:#075c3b}.auth-social{display:flex;gap:8px;margin-top:8px}.auth-social button{margin-top:0;flex:1}.auth-google{background:#fff!important;color:#1f2937!important;border:1px solid #d6ddd9!important}.auth-apple{background:#111!important;color:#fff!important}.auth-box .danger{background:#f8e9e7;color:#a22}
  ...
  const menu=document.createElement('div');menu.className='auth-menu';menu.innerHTML='<div style="padding:10px 12px 8px;color:#65736d;font-size:12px;line-height:1.35">🤝 <strong style="color:#075c3b">Con il tuo account puoi contribuire al sito</strong><br>Recensioni, parcheggi, camper service e prezzi carburante.</div><button data-action="profile">👤 Il mio profilo</button><button data-action="reviews">⭐ Le mie recensioni</button><button data-action="contribute">🤝 Contribuisci al sito</button><button data-action="name">✏️ Modifica username</button><button class="logout" data-action="logout">🚪 Esci</button>';document.body.appendChild(menu);

  const modal=document.createElement('div');modal.className='auth-modal';
  modal.innerHTML='<div class="auth-box" role="dialog" aria-modal="true"><button class="auth-close" type="button" aria-label="Chiudi">×</button><h2 id="authTitle">Accedi</h2><p id="authText">Accedi per lasciare recensioni.</p><div id="authFields"></div><button id="authSubmit" type="button">ACCEDI</button><button id="authSwitch" class="alt" type="button">REGISTRATI</button><div class="auth-social"><button id="authGoogle" class="auth-google" type="button">G Continua con Google</button><button id="authApple" class="auth-apple" type="button"> Continua con Apple</button></div><button id="authForgot" class="auth-link" type="button">Password dimenticata?</button><div class="auth-msg" id="authMsg" aria-live="polite"></div></div>';
  ...
  async function ensureProfile(user){if(!user)return null;const p=await s.from('profiles').select('id,display_name,avatar_url').eq('id',user.id).maybeSingle();if(p.data)return p.data;const name=user.user_metadata?.display_name||user.user_metadata?.full_name||user.user_metadata?.name||user.email?.split('@')[0]||'Utente';const ins=await s.from('profiles').insert({id:user.id,display_name:name});return ins.error?null:{id:user.id,display_name:name,avatar_url:null}}
  async function getProfile(user){
    user=user||await currentUser();if(!user)return null;await ensureProfile(user);
  ...
  async function render(){
    ...
    if(mode==='register'){title.textContent='Registrati';text.textContent='Crea gratuitamente il tuo account: potrai recensire ristoranti, parcheggi e camper service e segnalare i prezzi carburante che trovi durante i tuoi viaggi.';social.style.display='flex';fields().innerHTML=input('authName','text','Nome visualizzato')+input('authEmail','email','Email')+input('authPassword','password','Password (min. 6 caratteri)');submit.textContent='REGISTRATI';sw.textContent='HO GIÀ UN ACCOUNT';submit.style.display=sw.style.display='block';forgot.style.display=logout.style.display='none';return}
    if(mode==='forgot'){social.style.display='none';title.textContent='Recupera password';text.textContent='Inserisci la tua email: ti invieremo un link per scegliere una nuova password.';fields().innerHTML=input('authEmail','email','Email');submit.textContent='INVIA EMAIL';sw.textContent='TORNA ALL’ACCESSO';submit.style.display=sw.style.display='block';forgot.style.display=logout.style.display='none';return}
    if(mode==='reset'){social.style.display='none';title.textContent='Nuova password';text.textContent='Scegli una nuova password per il tuo account.';fields().innerHTML=input('authPassword','password','Nuova password (min. 6 caratteri)');submit.textContent='SALVA NUOVA PASSWORD';sw.style.display=forgot.style.display=logout.style.display='none';submit.style.display='block';return}
    title.textContent='Accedi';text.textContent='Con un account puoi lasciare recensioni e contribuire al sito: segnala prezzi carburante aggiornati, valuta parcheggi, camper service e ristoranti e ritrova i tuoi contributi nel profilo.';social.style.display='flex';fields().innerHTML=input('authEmail','email','Email')+input('authPassword','password','Password');submit.textContent='ACCEDI';sw.textContent='REGISTRATI';submit.style.display=sw.style.display=forgot.style.display='block';logout.style.display='none';
  }
  ...
  async function socialLogin(provider){msg('Apertura accesso '+(provider==='google'?'Google':'Apple')+'...');const r=await s.auth.signInWithOAuth({provider,options:{redirectTo:redirectUrl}});if(r.error)msg(r.error.message)}
  q('#authGoogle').onclick=()=>socialLogin('google');q('#authApple').onclick=()=>socialLogin('apple');

(function(){
  ...
  async function render(){
    ...
    if(mode==='register'){title.textContent='Registrati';text.textContent='Crea gratuitamente il tuo account: potrai recensire ristoranti, parcheggi e camper service e segnalare i prezzi carburante che trovi durante i tuoi viaggi.';fields().innerHTML=input('authName','text','Nome visualizzato')+input('authEmail','email','Email')+input('authPassword','password','Password (min. 6 caratteri)');submit.textContent='REGISTRATI';sw.textContent='HO GIÀ UN ACCOUNT';submit.style.display=sw.style.display='block';forgot.style.display=logout.style.display='none';return}
    ...
    title.textContent='Accedi';text.textContent='Con un account puoi lasciare recensioni e contribuire al sito: segnala prezzi carburante aggiornati, valuta parcheggi, camper service e ristoranti e ritrova i tuoi contributi nel profilo.';fields().innerHTML=input('authEmail','email','Email')+input('authPassword','password','Password');submit.textContent='ACCEDI';sw.textContent='REGISTRATI';submit.style.display=sw.style.display=forgot.style.display='block';logout.style.display='none';
  }
